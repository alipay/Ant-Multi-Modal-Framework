...
from antmmf.utils.distributed import is_master, synchronize
...
class DownloadableFile:
    ...
    def _parse_url(self, url):
        if url.find(self.ANTMMF_PREFIX) == -1:
            return url
        else:
            return self.ANTMMF_PREFIX + url[len(self.ANTMMF_PREFIX) :]
...
def download_based_on_config(config, resources, download_path, version):
    def download_on_attribute(resources, download_path, version, attribute):
        path = os.path.join(download_path, attribute)
        download_resources(resources.get(attribute, []), path, version)
    ...
    if not isinstance(resources, collections.abc.Mapping):
        ...
        if use_features:
            download_on_attribute(resources, download_path, version, "features")

        if use_images:
            download_on_attribute(resources, download_path, version, "images")

        download_on_attribute(resources, download_path, version, "annotations")
        download_resources(resources.get("extras", []), download_path, version)
