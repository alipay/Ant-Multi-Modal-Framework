...
to add dataset builder to ease setup of antmmf.

``_load`` is used to load a dataset from specific path. ``_load`` needs to return
an instance of subclass of ``antmmf.tasks.base_dataset.BaseDataset``.
...
class BaseDatasetBuilder:
    ...
    def load(self, dataset_type, config, *args, **kwargs):
        """Main load function use by antmmf. This will internally call ``_load``
        ...
        Args:
            ...
            config (Configuration): Configuration of this dataset loaded from config.
    ...
    def build(self, dataset_type, config, *args, **kwargs):
        """
        Similar to load function, used by antmmf to build a dataset for first
        time when it is not available. This internally calls '_build' function.

        Args:
            dataset_type (str): Type of dataset, train|val|test
            config (Configuration): Configuration of this dataset loaded from
                                 config.
    ...
    def update_registry_for_model(self, config):
        registry.register(constants.DATASET_NAME, self.dataset_name)
