...
from antmmf.modules.embeddings import BertVisionLinguisticEmbeddings
...
class VisualBERTBase(BertPreTrainedModel):
    def __init__(
        ...
        self.embeddings = BertVisionLinguisticEmbeddings(config)
...
class VisualBERT(BaseModel):
    ...
    def flatten(self, sample_list, to_be_flattened=None, to_be_flattened_dim=None):
        ...
        if sample_list.image_mask is not None:
            ...
            nbr_img = sample_list.image_feature_0.size(-2)
            bsz = sample_list.input_mask.size(0)
            image_mask = torch.arange(
                nbr_img, device=sample_list.input_mask.device
            ).expand((bsz, -1))

            image_mask = torch.ones(
                (sample_list.input_mask.size(0), nbr_img), dtype=torch.long
            )
            image_mask.to(device=sample_list.input_mask.device)
