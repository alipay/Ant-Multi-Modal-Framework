...
from antmmf.modules.layers import Linear
...
class TopDownBottomUp(BaseModel):
    def __init__(self, image_attention_model, text_embedding_models, classifier):
        ...
        self.non_linear_text = Linear(text_lstm_dim, joint_embedding_out_dim)
        self.non_linear_image = Linear(image_feat_dim, joint_embedding_out_dim)
    ...
    def forward(
        ...
        text_embedding = F.relu_(self.non_linear_text(text_embedding))
        image_embedding = F.relu_(self.non_linear_image(image_embedding))
        joint_embedding = text_embedding * image_embedding
