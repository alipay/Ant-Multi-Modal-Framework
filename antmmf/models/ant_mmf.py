...
from antmmf.modules.layers import ModalCombineLayer
...
class AntMMF(BaseModel):
    ...
    def _init_text_embeddings(self, attr="text"):
        ...
        for text_embedding in text_embeddings_list_config:
            ...
            embedding_kwargs = Configuration(text_embedding.params)
    ...
    def _init_feature_encoders(self, attr):
        ...
        for feat_encoder in feat_encoders_list_config:
            ...
            feat_model = VisualEncoder(encoder_type, feature_dim, **encoder_kwargs)
    ...
    def process_feature_embedding(
        ...
        assert len(features) == len(
            feature_encoders
        ), "Number of feature encoders, {} are not equal to number of features, {}.".format(
            len(feature_encoders), len(features)
        )
        ...
        for i, feature in enumerate(features):
            ...
            # For antmmf, we need max_features to mask attention
    ...
    def forward(self, sample_list):
        ...
        model_output = {"logits": self.calculate_logits(joint_embedding)}

        return model_output


# TODO: Update
@registry.register_model("antmmf_question_only")
class AntMMFQuestionOnly(AntMMF):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(self, sample_list):
        ...
        model_output = {"logits": scores}

        return model_output


# TODO: Update
@registry.register_model("antmmf_image_only")
class AntMMFImageOnly(AntMMF):
    def __init__(self, config):
        super().__init__(config)
