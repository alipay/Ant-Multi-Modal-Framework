...
def eval_model(args):
    ...
    for batch_id in tqdm(range(total_items // batch_size + 1)):
        ...
        batch_pormpts_point = {'obj_question': [], 'super_question': [], 'general_question': []}
        batch_pormpts_bbox = {'obj_question': [], 'super_question': [], 'general_question': []}
        batch_images = []
        batch_has_images = []
        result_dicts = []
        ...
        for i, line in enumerate(batch_questions):
            ...
            scaled_point = [line['point']['x'] * scaled_width, line['point']['y'] * scaled_height]
            scaled_bbox = [line['bbox']['x'] * scaled_width, line['bbox']['y'] * scaled_height, (line['bbox']['x'] + line['bbox']['w']) * scaled_width, (line['bbox']['y'] + line['bbox']['h']) * scaled_height]
            ...
            for select_type in question_type:
                select_question = line[select_type]
                question = "{} {}{}{}".format(select_question, BEGIN_LOC, location_token_point, END_LOC)
                bbox_question = "{} {}{}{}".format(select_question, BEGIN_LOC, location_tokens_bbox, END_LOC)
                ...
                batch_pormpts_point[select_type].append(cur_prompt_point)
                batch_pormpts_bbox[select_type].append(cur_prompt_bbox)
        ...
        for select_type in question_type:
            tokenized_output_point = tokenizer(
                batch_pormpts_point[select_type],
                return_tensors="pt",
                padding="longest",
            ...
            tokenized_output_bbox = tokenizer(
                batch_pormpts_bbox[select_type],
                return_tensors="pt",
                padding="longest",
                max_length=tokenizer.model_max_length,
                truncation=True,
            ...
            for i in range(len(outputs_bbox)):
                result_dicts[i].update({
                                    "{}_answer_bbox".format(select_type): outputs_bbox[i],
                                    "{}_answer_point".format(select_type): outputs_point[i],
                                    })
        for i in range(len(result_dicts)):
            ans_file.write(json.dumps(result_dicts[i]) + "\n")
        ans_file.flush()
    ans_file.close()
    os.system("python pink/eval/eval_pointingvqa_looktwice.py --result-file {}".format(args.answers_file))
