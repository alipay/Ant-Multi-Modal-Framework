...
def eval_model(args):
    ...
    questions_dict = json.load(open(args.question_file, "r"))
    questions = []
    ...
    if args.strict_answer:
        merge_dataset = questions
        answer_list = []
        for t in tqdm(merge_dataset):
            answer_list.append(t['answer'])
            answer_list.append(t['fullAnswer'])
        candidate_answer = [(k + DEFAULT_EOS_TOKEN) for k, v in Counter(answer_list).items() if v >= 3]
    ...
    for batch_id in tqdm(range(total_items // batch_size + 1)):
        ...
        for i, line in enumerate(batch_questions):
            ...
            gt_ans = line['answer']
            full_gt_ans = line['fullAnswer']
            try:
                with open(os.path.join(args.image_folder, "{}.jpg".format(idx)), "rb") as f:
            ...
            result_dict['full_gt_answer'] = full_gt_ans
