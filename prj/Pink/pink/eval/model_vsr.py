...
def eval_model(args):
    ...
    for batch_id in tqdm(range(total_items // batch_size + 1)):
        ...
        for i, line in enumerate(batch_questions):
            ...
            try:
                if "val2017" in line['image_link']:
                    sub_dir = "val2017"
                else:
                    sub_dir = "train2017"
                with open(os.path.join(args.image_folder, sub_dir, image_id), "rb") as f:
            ...
            question_prompt = question + " Is it correct? Answer with Yes or No."
            select_question = random.choice(QuestionAnswer)
            select_question = select_question.replace(" <image>", "")
            select_question = select_question.replace("<question>", "{}{}{}".format(BEGIN_QUESTION, question_prompt, END_QUESTION))
            ...
            result_dict['gt_answer'] = line['label']
        ...
        for i in range(len(outputs)):
            if "yes" in outputs[i].lower()  and result_dicts[i]['gt_answer'] == 1:
                num_correct += 1
            elif "no" in outputs[i].lower() and result_dicts[i]['gt_answer'] == 0:
                num_correct += 1
